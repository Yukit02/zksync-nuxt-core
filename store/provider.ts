...
import { zkSyncNetworkConfig, ethereumNetworkConfig } from "../utils/config";
...
export const actions: ActionTree<ProviderState, ProviderState> = {
  ...
  async changeNetwork({ commit, dispatch, rootGetters }, networkName: ZkEthereumNetworkName) {
    ...
    if (rootGetters["zk-onboard/onboardInited"]) {
      rootGetters["zk-onboard/onboard"]?.config({
        networkId: ethereumNetworkConfig("")[zkSyncNetworkConfig[networkName].ethereumNetwork].id,
      });
    }
    cache.clear();
    try {
    ...
    if (rootGetters["zk-account/loggedIn"]) {
      ...
      if (rootGetters["zk-onboard/selectedOnboardType"] === "Onboard") {
        await dispatch("zk-onboard/loginWithOnboard", rootGetters["zk-onboard/selectedWallet"], { root: true });
      } else if (rootGetters["zk-onboard/selectedOnboardType"] === "WalletConnect") {
        await dispatch("zk-onboard/walletConnectLogin", null, { root: true });
      }
    } else {
