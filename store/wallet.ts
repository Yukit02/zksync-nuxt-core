...
import { ModuleOptions, ZkCPKStatus, ZkCPKLocal, ZkSignCPKState } from "../types";
...
export const actions: ActionTree<ProviderState, ProviderState> = {
  ...
  async openWalletApp({ rootGetters }) {
    if (rootGetters["zk-onboard/selectedOnboardType"] !== "WalletConnect") {
      return;
    }
    const wcURI = rootGetters["zk-onboard/providerWalletConnect"].wc.uri;
